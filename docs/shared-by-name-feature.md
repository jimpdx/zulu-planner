...
- Add a new field for "Created By" input field and rerrange the columns as follows:
  - Left Column: Event Name, Created By, Departure Start, Arrival Offset
  - Right Column: Event Date, Average Flight Duration, Arrival Start Default Shift Length
- Note that Created By should render as "Shared by" when the plan is loaded from a share link
...
- When a shared plan loads successfully and has a `sharedBy` value, surface it to the UI by pre-populated the "Created By" field with the value but changed "Created by to "Shared by"
